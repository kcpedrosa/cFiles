...
int main(int argc, char *argv[])
{
    ...
    do
    {
        ...
        printw("would you like to add a book to the library?\n If yes, press b");
        ...
        {
            msgstatus(win, "Are you sure you want to get out?(y/n)", STYESNO);  /*chama a funcao msgstatus()*/
        }
...
void padd(wins_t *pw)
{
    ...
    FILE *file; /*Arquivo*/
    char novoLivro[20]; /*String com nome do Livro*/
    int lst = 8; /*Linha que inicia*/
    ...
    mvwscanw(pw->w[pw->wi], lst++, 1, "%s", novoLivro);  /*Escreve no arquivo*/

    fputs(novoLivro, file); /*Insere o que foi escrito no arquivo*/
...
int msgstatus(wins_t win, char *msg, int tipo)
{
    ...
    switch(tipo)
    {
        ...
        case STYESNO:
             ...
             if(kpress == 'y')
             {
                bomb(win, "", 0, NULL); /*finaliza programa*/
             }
             else
             {
                 mvwhline(win.w[win.wi], STLINE, 1, ' ', COLS-2);//LD
             }
             break;
         default:
