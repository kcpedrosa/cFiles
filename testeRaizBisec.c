...
double _raiz(double _numero);
double _numero = 0;
double _y = 0;
double _p = 0;

int main(void) {
   imprima("s", "De que numero deseja obter a raiz:");
   _numero = leia_real();
   imprima("sf", "A raiz de numero eh:", _raiz(_numero));
   cleanup();
   return EXIT_SUCCESS;
}
double _raiz(double _numero) {
   _y = _numero;
   while(((_numero-_p)>0.0001)) {
      _numero = ((_numero+_p)/2);
      _p = (_y/_numero);
   }
   return _numero;
}